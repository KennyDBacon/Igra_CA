...
\code
// WARNING: C#/Java code, will not work in C++
SomeClass obj = ...;
...
// This WILL work in C++
SomeClass obj = ...;
wstring s = ToString(obj) + L" a string";
...
// This will work in C++
SomeClass obj = ...;
wstring s = ToString(obj," a string",3.142);	// s=obj+"a string"+3.142
DrawD3DFont(font,s,0,0,WHITE_COL);
\endcode
The functions return an std::string, so if you want to convert into a wchar*
...
*/
template<class T>
inline std::wstring ToString(const T& t1)
{
...
inline std::wstring ToString(const T1& t1,const T2& t2)
{
	std::wstringstream sout;
	sout<<t1<<t2;
	return sout.str();
...
inline std::wstring ToString(const T1& t1,const T2& t2,const T3& t3)
{
	std::wstringstream sout;
	sout<<t1<<t2<<t3;
	return sout.str();
...
inline std::wstring ToString(const T1& t1,const T2& t2,const T3& t3,const T4& t4)
{
	std::wstringstream sout;
	sout<<t1<<t2<<t3<<t4;
	return sout.str();
...
static const wchar_t* format(const wchar_t* fmt, ... )
{
	...
	_vsnwprintf_s(buff,256,fmt,args);	// put data into buffer
	va_end(args);
	return buff;
}

static const char* formatA(const char* fmt, ... )
{
	static char buff[256];
	va_list args;			// declaring a argument list
	va_start(args, fmt);		// initialise the argument list
	_vsnprintf_s(buff,256,fmt,args);	// put data into buffer
	va_end(args);
	return buff;
}
